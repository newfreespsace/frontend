return {
  ...
  preference: {
    problem_review: {
      ...
      enabled: "启用题目复习",
      enabled_notes:
        "默认关闭。开启并保存后，首次通过的新题目会加入复习计划；历史已通过题目不会自动加入。关闭后保留进度，停止生成和推进计划；重新开启后日期不顺延，可能出现逾期题目。",
      count: "复习次数",
      round: "第 {number} 轮",
      available_days: "开始复习的间隔天数",
      overdue_days: "开始逾期的间隔天数",
      timing_notes:
        "第一轮从首次通过日期计算，后续轮次从上一次复习完成日期计算。间隔不含完成当天，经过指定数量的完整自然日后，于服务器时区的零点生效。例如 9 月 5 日完成、间隔 1 天，则 9 月 7 日 00:00 开始。",
      invalid_schedule: "请设置 1～10 轮复习，天数须为 1～365 的整数，且每轮逾期天数必须大于开始天数。",
      restore_defaults: "恢复默认的 4 轮时间",
      changes_notes:
        "保存后，未完成计划立即按新规则重算日期，保留已完成轮数；达到新次数的计划会标记完成。已完成的计划不会因增加次数而重新开启。"
    },
    title: "偏好设置",
    locale: {
      header: "语言",
