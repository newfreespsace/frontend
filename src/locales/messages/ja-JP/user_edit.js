return {
  ...
  preference: {
    problem_review: {
      ...
      enabled: "問題の復習を有効にする",
      enabled_notes:
        "初期設定は無効です。有効にして保存すると、新たに初めて正解した問題が復習計画に追加されます。過去に正解した問題は追加されません。無効にすると進捗を保持したまま計画の作成・進行が停止します。再度有効にしても日付は延長されないため、期限切れになる場合があります。",
      count: "復習回数",
      round: "第 {number} 回",
      available_days: "復習開始までの日数",
      overdue_days: "期限切れまでの日数",
      timing_notes:
        "初回は最初の正解日、以降は前回の復習完了日を基準にします。完了当日を除き、指定した日数が経過した後、サーバーのタイムゾーンの午前 0 時に開始します。例えば 9 月 5 日に完了し、間隔が 1 日なら、9 月 7 日の午前 0 時に開始します。",
      invalid_schedule:
        "復習回数は 1～10 回、日数は 1～365 の整数にしてください。各回の期限切れまでの日数は、開始までの日数より大きくしてください。",
      restore_defaults: "標準の 4 回の設定に戻す",
      changes_notes:
        "保存すると未完了の計画の日付が再計算され、完了済みの回数は保持されます。新しい回数に達した計画は完了となります。回数を増やしても完了済みの計画は再開されません。"
    },
    title: "環境設定",
    locale: {
      header: "言語",
